...
export const listMySchedules = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`/api/schedules/ongoing/all/${id}`, config)
...
export const listCompletedSchedules = () => async (dispatch, getState) => {
  try {
    dispatch({
      type: ALL_COMPLETED_SCHEDULES_REQUEST,
    })
    ...
    const { data } = await axios.get(`/api/schedules/completed/all`, config)

    dispatch({
      type: ALL_COMPLETED_SCHEDULES_SUCCESS,
      payload: data,
    })
  } catch (error) {
    dispatch({
      type: ALL_COMPLETED_SCHEDULES_FAIL,
      payload:
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message,
