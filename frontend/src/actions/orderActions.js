...
//list orders
export const emplistOrders = () => async (dispatch, getState) => {
  try {
    dispatch({
      type: ORDER_LIST_EMP_REQUEST,
    })
    ...
    const { data } = await axios.get(`/api/orders/emp`, config)

    dispatch({
      type: ORDER_LIST_EMP_SUCCESS,
      payload: data,
    })
  } catch (error) {
    dispatch({
      type: ORDER_LIST_EMP_FAIL,
      payload:
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message,
