...
export const listNotPaidOrders = (orderId, paymentResult) => async (
  dispatch,
  getState
) => {
  try {
    dispatch({
      type: NOT_PAID_ORDER_LIST_REQUEST,
    });
    ...
    const { data } = await axios.get(`/api/orders/notpaidorders`, config);

    dispatch({
      type: NOT_PAID_ORDER_LIST_SUCCESS,
      payload: data,
    });
  } catch (error) {
    dispatch({
